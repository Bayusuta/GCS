...
function addPointLayerSource(lon, lat){
  ...
  savedMissionPoints.push([JSON.parse(JSON.stringify(newFeature)).ol_uid, lat, lon]);
...
function latChanged(input){
  ...
  var lat=input.value;
  var lon=savedMissionPoints[index][2];

  savedMissionPoints[index] = [ol_uid, lat, lon];
...
function lonChanged(input){
  ...
  var lat=savedMissionPoints[index][1];
  var lon=input.value;
...
$(".delete-row").click(function(){
  ...
  for(i=0; i<duplicate_savedMissionPoints.length; i++){
    addPointLayerSource(duplicate_savedMissionPoints[i][2], duplicate_savedMissionPoints[i][1]);
  }
